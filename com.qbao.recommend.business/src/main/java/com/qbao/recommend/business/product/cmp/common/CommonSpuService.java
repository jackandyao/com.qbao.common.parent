...
public  class CommonSpuService extends AbstractRecommedService {
    
    Logger spuServiceLogger = LoggerManagerUtil.getLogger(LogNameEnum.spu_service, Level.WARN);
    ...
    protected List<Long>hotSpuList; 
	 ...
	 * @return
	 */
	protected void initHotSpuList() {
		hotSpuList = hotSpuService.findAllSpuIdofHotSpuMerchant();
	}
	...
	private String getInfo(String type) {
		if (type.equals(RecommendConstantUtil.IS_SIMILAR)) {
			return "相似";
		} else {
			return "类目关联";
		}
	...
	private void initTagKeyDir() {
		if (isInit) {
			List<SpuInfoMerchant> tagKeyDirList = merMiddleDataService.findTagKeyDirectoryList();
			for (SpuInfoMerchant info : tagKeyDirList) {
				String spuId = String.valueOf(info.getSpuId());
				int lev = info.getTagKeyDirectory();
				tagKeyDirMap.remove(spuId);
				tagKeyDirMap.put(spuId, lev);
			}
			isInit = false;
		}
	}
	
	protected List<Long> splitSpuList(List<Long> oldList,int num){
	    if(oldList.size()<=num){
	        return oldList;
	    }else{
