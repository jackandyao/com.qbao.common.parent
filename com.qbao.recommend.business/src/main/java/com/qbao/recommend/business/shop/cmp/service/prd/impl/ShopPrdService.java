package com.qbao.recommend.business.shop.cmp.service.prd.impl;
...
@Service
public class ShopPrdService extends CommonShopService implements IShopPrdService {
    ...
    @Autowired
    IRecShopPrdService recShopPrdService;
    
    @Override
    public List<String> getTopShopByPrd(JSONObject jsonObject) {
        ...
        try{
            RecommendShop recommentShop= recShopPrdService.findByUserId(userId);
            if(recommentShop != null) resultList = recommentShop.getShopList();
        }catch(Exception e){
            // 发送邮件
            this.servericeLogger.warn("ShopPrdService.getTopShopByPrd  recommentItems is null! userId:" + userId);
        ...
        if (resultList.size() < num && !StringUtils.isEmpty(completions)) {
            this.servericeLogger.warn("ShopPrdService.getTopShopByPrd The data is not enough! request_num:" + num + " reality_num:" + resultList.size() + "  Completion strategy:" + completions);
            jsonObject.put(MessageConstanUtil.ALARM_TYPE, MessageConstanUtil.MAIL_ERROR);
            jsonObject.put(MessageConstanUtil.ALARM_MESSAGE, "ShopPrdService.getTopShopByPrd the data is not enough! request_num:" + num + " reality_num:" + resultList.size() + "  Completion strategy:" + completions);
            jsonObject.put(MessageConstanUtil.ALARM_USERID, userId);
