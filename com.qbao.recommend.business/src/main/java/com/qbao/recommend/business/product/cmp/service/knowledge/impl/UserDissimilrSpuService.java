package com.qbao.recommend.business.product.cmp.service.knowledge.impl;
...
import com.qbao.recommend.respositoy.mysql.service.IRecUserDisService;
 ...
 */
@Service
public class UserDissimilrSpuService extends CommonSpuService implements IUserDisimilrSpuService {
    ...
    @Autowired
    IRecUserDisService recUserDisService;
    
    @Override
    public List<Long> getUserSimilarSpuTop(JSONObject jsonObject) {
        ...
        try{
            RecommendItems recommentItems = recUserDisService.findByUserId(userId);
            resultList = recommentItems.calSpuIdsList();
        }catch(Exception e){
             // 发送邮件
            this.servericeLogger.warn("UserDissimilrSpuService.getUserSimilarSpuTop error! userId:" + userId);
        ...
        if (resultList.size() < num&&!StringUtils.isEmpty(completions)) {
            this.servericeLogger.warn("UserDissimilrSpuService.getUserSimilarSpuTop The data is not enough! request_num:" + num + " reality_num:" + resultList.size() + "  Completion strategy:" + completions);
            Exception e = new Exception("UserDissimilrSpuService.getUserSimilarSpuTop The data is not enough! request_num:" + num + " reality_num:" + resultList.size() + "  Completion strategy:" + completions);
            jsonObject.put(MessageConstanUtil.ALARM_EXCEPTION, e);
            jsonObject.put(MessageConstanUtil.ALARM_TYPE, MessageConstanUtil.MAIL_ERROR);
            jsonObject.put(MessageConstanUtil.ALARM_USERID, userId);
            AlaramServiceFacotryFacade.sendAlaramMessageByMail(jsonObject);
